...
class shippers extends DbTable
{
	...
	public function __construct()
	{
		...
		// ShipperID
		$this->ShipperID = new DbField('shippers', 'shippers', 'x_ShipperID', 'ShipperID', '`ShipperID`', '`ShipperID`', 3, -1, FALSE, '`ShipperID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// CompanyName
		$this->CompanyName = new DbField('shippers', 'shippers', 'x_CompanyName', 'CompanyName', '`CompanyName`', '`CompanyName`', 200, -1, FALSE, '`CompanyName`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->CompanyName->Sortable = TRUE; // Allow sort
		$this->fields['CompanyName'] = &$this->CompanyName;

		// Phone
		$this->Phone = new DbField('shippers', 'shippers', 'x_Phone', 'Phone', '`Phone`', '`Phone`', 200, -1, FALSE, '`Phone`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	public function insert(&$rs)
	{
		...
		if ($success) {
			...
			$this->ShipperID->setDbValue($conn->insert_ID());
	...
	public function getRecordFilter($row = NULL)
	{
		...
		$val = is_array($row) ? (array_key_exists('ShipperID', $row) ? $row['ShipperID'] : NULL) : $this->ShipperID->CurrentValue;
		if (!is_numeric($val))
	...
	{
		global $Language;
		if ($pageName == "shippersview.php")
			return $Language->phrase("View");
		elseif ($pageName == "shippersedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "shippersadd.php")
	...
	{
		if ($parm <> "")
			$url = $this->keyUrl("shippersview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("shippersview.php", $this->getUrlParm(TABLE_SHOW_DETAIL . "="));
	...
	{
		if ($parm <> "")
			$url = "shippersadd.php?" . $this->getUrlParm($parm);
		else
			$url = "shippersadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("shippersedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("shippersadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("shippersdelete.php", $this->getUrlParm());
	}
	...
	public function keyUrl($url, $parm = "")
	{
		...
		if ($this->ShipperID->CurrentValue != NULL) {
			$url .= "ShipperID=" . urlencode($this->ShipperID->CurrentValue);
		} else {
	...
	public function getRecordKeys()
	{
		...
		if (Param("key_m") !== NULL) {
			...
			if (Param("ShipperID") !== NULL)
				$arKeys[] = Param("ShipperID");
	...
	{
		$this->ShipperID->setDbValue($rs->fields('ShipperID'));
